/*
 * DUCO GPU Miner for SGI Workstations - FULLY PATCHED STABLE VERSION
 * Thread-safe OpenGL, reduced GPU calls, fast reconnect, job expiry.
 * 
 * Compile on IRIX 6.5:
...
/* ============================================================================
 * OPENGL GLOBALS & MUTEX (THREAD‑SAFE)
 * ============================================================================ */
...
static pthread_mutex_t gl_mutex = PTHREAD_MUTEX_INITIALIZER;  /* GLOBAL FIX */
...
/* ============================================================================
 * HASH & TARGET MATCH (exactly like Rust miner)
 * ============================================================================ */
...
/* ============================================================================
 * NETWORK FUNCTIONS (with timeout fix)
 * ============================================================================ */
...
int recv_line(int sock, char *buf, size_t size) {
    ...
    while (i < size - 1) {
        FD_ZERO(&fdset);
        FD_SET(sock, &fdset);
        tv.tv_sec = 2;   /* FIX shorter timeout */
        tv.tv_usec = 0;
...
int get_pool_info(char *addr, int *port) {
    int sock = connect_with_timeout("server.duinocoin.com", 80, 10);
    if (sock < 0) return -1;
    char request[512];
...
void* mining_loop(void *arg) {
    ...
    while (state.running) {
        ...
        /* less GPU calls: only every 1000 nonces */
        ...
        /* expire old job after 5 minutes */
        pthread_mutex_lock(&job_mutex);
        if (current_job.active &&
...
/* ============================================================================
 * CONNECTION THREAD (manages pool)
 * ============================================================================ */
    ...
    while (state.running) {
        ...
        if (state.socket_fd >= 0) {
            ...
            while (state.running && state.socket_fd >= 0) {
                ...
                /* parse job line using fixed parser */
                char base[128], target_hex[41];
                unsigned int diff;
                if (parse_job_line(line, base, target_hex, &diff)) {
                    job_t new_job;
...
int main(int argc, char *argv[]) {
    ...
    printf("╔═══════════════════════════════════════════════════════════════╗\n");
    printf("║     DUCO GPU Miner for SGI - FULLY PATCHED STABLE           ║\n");
    printf("║     Thread-safe OpenGL | Reduced GPU calls | Fast reconnect ║\n");
    printf("╚═══════════════════════════════════════════════════════════════╝\n\n");
    ...
    printf("🚀 Starting miner... Press Ctrl+C to stop\n\n");
    ...
    /* THREAD SAFE CREATE – allocate per‑thread id on heap */
